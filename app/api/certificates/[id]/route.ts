...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!cert) return NextResponse.json({ ok: false, error: "No encontrado" }, { status: 404, headers: noStore() });
...
export async function POST(req: Request, ctx: any) {
  ...
  return NextResponse.json({ ok: false, error: "Método no soportado" }, { status: 405, headers: noStore() });
}